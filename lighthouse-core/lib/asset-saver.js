...
const getMetricEvents = require('./asset-saver-metrics');
  ...
  return passNames.reduce((chain, passName) => {
    ...
    return chain.then(_ => artifacts.requestScreenshots(trace))
      .then(screenshots => {
        ...
        const metricEvents = getMetricEvents(traceData.traceEvents, auditResults, options);
        traceData.traceEvents.push(...metricEvents);

        assets.push({
          traceData,
          html: screenshotDump(options, screenshots)
        });
      });
  }, Promise.resolve())
    .then(_ => assets);
